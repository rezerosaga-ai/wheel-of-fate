...
class Player:
    ...
    async def pass_age_gate(self):
        ...
        # try exact phrase first, then the longer label with emoji, then any button containing "أكبر"
        gate = (
            self.page.get_by_text("نعم، أنا أكبر من 17", exact=False)
            .or_(self.page.get_by_text("نعم، أنا أكبر من 17 سنة"))
            .or_(self.page.get_by_text("أكبر من 17"))
        )
        waited = 0
        for _ in range(20):
            if await gate.count() > 0:
                ...
                await self.page.wait_for_timeout(3000)
                self.log("age gate passed", {"waited_ms": waited})
                return
            await self.page.wait_for_timeout(1000)
            waited += 1000
            if waited >= 30000:
                break
        # final fallback: if the home screen is already visible, skip the gate
        home = self.page.get_by_text("ابدأ لعبة جديدة")
        if await home.count() > 0:
            self.log("age gate: skipped — home already visible")
            return
        self.log("age gate: not found after wait")

    async def set_name_and_mood(self, confirm=True):
        """من شاشة home: ابدأ لعبة جديدة -> اسم + مزاج -> إنشاء غرفة."""
        # wait for the home screen content to be hydrated (router chunks may still load)
        started = False
        for i in range(10):
            try:
                await self.page.get_by_text("ابدأ لعبة جديدة").first.click(timeout=5000)
                started = True
                break
            except Exception:
                self.log("start-click retry", {"i": i})
                await self.page.wait_for_timeout(1500)
        if not started:
            try:
                await self.page.screenshot(path=f"/tmp/harness-{self.name}-fail.png")
            except Exception:
                pass
            raise RuntimeError(f"{self.name}: زر ابدأ لعبة جديدة لم يظهر بعد {i+1} محاولات")
        await self.page.wait_for_timeout(3000)
    ...
    async def react_emoji(self, emoji="❤️"):
        """إذا ظهر اختيار reactions — يطابق الزر الذي يبدأ نصه بالإيموجي (الزر يحتوي label أيضًأ)."""
    ...
    async def react_weak(self):
        """رد ضعيف (مرحلة E2 — Conflict): 😢/🥶/😲 — أول زر weak ظاهر."""
        for e in ["😢", "🥶", "😲"]:
            if await self.react_emoji(e):
                return e
        return None
...
async def run_main(mode):
    ...
    async with async_playwright() as pw:
        ...
        try:
            # ابحث عن زر فئة/سؤال — نص «اختر السؤال»
            txt = await abdo.visible_text()
            st3["abdo_visible"] = txt[:400]
            if await abdo.page.get_by_text("اختر السؤال").count() > 0:
                await abdo.click_text("اختر السؤال", wait_after=3000)
                await abdo.snapshot("picked-question")
                st3["picked"] = True
            else:
                st3["picked"] = False
                st3["status"] = "BLOCKED"; st3["reason"] = "لم يظهر زر اختيار السؤال"
            if st3["picked"]:
                st3["status"] = "PASS"
        except asyncio.TimeoutError as e:
        ...
        try:
            ...
            # أنفال تجيب (إذا كان دورها)
            t_anfal = await anfal.visible_text()
            if "دورك" in t_anfal:
                await anfal.click_text("سؤال تالي", wait_after=0) if False else None
                # ابحث عن زر تأكيد/إرسال
                btns = ["تأكيد", "إرسال الإجابة", "أرسل"]
                hit = None
                for b in btns:
                    if await anfal.page.get_by_text(b).count() > 0:
                        hit = b; break
                st4["submit_btn"] = hit
                await anfal.snapshot("answer-ready")
            st4["status"] = "BLOCKED"  # لا نُدّعي PASS — الإجابة عبر UI تعتمد على حالة الغرفة
        except asyncio.TimeoutError as e:
        ...
        try:
            ...
            for vp in [{"width": 375, "height": 812}, {"width": 412, "height": 915}, {"width": 1280, "height": 720}]:
                ...
                await p.join_as(name="أنفال", code=code)
                s = await p.state()
                s = s or {}
                await p.snapshot("lobby-vp")
                await p.dom("lobby-vp")
                st8["shots"].append({"vp": f"{vp['width']}x{vp['height']}", "url": s.get("url", "unknown"), "text": (s.get("text") or "")[:200]})
                await p.close()
            st8["status"] = "PASS" if all(sh["url"].startswith("/room/") for sh in st8["shots"]) else "FAIL"
        except asyncio.TimeoutError as e:
